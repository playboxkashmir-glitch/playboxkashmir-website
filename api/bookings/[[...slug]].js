...
// and confirmation email sending into one dynamic catch-all route file to
// stay within the Vercel Hobby plan's 12-function limit. Public paths are
// unchanged: /api/bookings, /api/bookings/:id, /api/bookings/availability,
...
export default async function handler(req, res) {
  ...
  if (parts[0] === 'availability') return handleAvailability(req, res);
  if (parts[0] === 'send-confirmation') return handleSendConfirmation(req, res);
...
async function handleAvailability(req, res) {
  ...
  try {
    ...
    return res.status(200).json({ blocked: rows });
  } catch (err) {
